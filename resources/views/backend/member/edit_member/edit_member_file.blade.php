@extends('backend.app')
@section('title','Update member Photos & other file')
@push('css')
...
@section('active_menu','Update Photos & other file')
@section('content')
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('admin.member/passport_photo_upload') }}" enctype="multipart/form-data" id="photograph_form">
                            ...
                            <div class="progress mt-2">
                                <div class="progress-bar css_1" role="progressbar" aria-valuenow="" aria-valuemin="0" aria-valuemax="100" ></div>
                                    0%
                                </div>
                            </div>
                            <div id="success"><img src="{{asset("uploads/member_Photograph/".$member->photo)}}" class="img-thumbnail css_2"/></div>
                            <span id="error_photograph_file_name" class="text-danger error_field"></span>
                            <span id="photograph_file_name">{{$member->photo}}</span>
                        </form>
                    </div>
                </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('admin.member/signature_upload') }}" enctype="multipart/form-data" id="sig_form">
                            ...
                            <div id="success"><img src="{{asset("uploads/member_sigh/".$member->signature)}}" class="img-thumbnail css_2" /></div>
                            <span id="sign_file_name">{{$member->signature}}</span>
                        </form>
...
{{--                                    </div>--}}
{{--                                </div>--}}
{{--                                <div id="success"><img src="{{asset("uploads/member_qu_code/".$member->army_qr_code_image)}}" class="img-thumbnail" style="height:100px;--}}
{{--                                margin-top:20px"/></div>--}}
{{--                                <span id="qr_code_file_name">{{$member->army_qr_code_image}}</span>--}}
{{--                            </form>--}}
...
{{--                @endif--}}
            ...
            <div class="row justify-content-center">
                <button class="btn btn-primary btn-lg btn-block col-md-6" type="button" style="margin-bottom: 10px; margin-right: 10px;" id="form_submission_button"
                        onclick="submit_other_document()">Update
                </button>
            </div>
        </div>
    </div>
...
@push('js')
    ...
    @include('backend.member.edit_member.edit_member_file_js')
@endpush
