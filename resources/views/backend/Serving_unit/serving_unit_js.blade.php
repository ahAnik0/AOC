<script nonce="{{ csp_nonce() }}">
    ...
    $(function () {
        var table = $('.yajra-datatable').DataTable({
            ...
            ajax: {
                url: "{{ url('admin/servingunit/search') }}",
                type: 'POSt',
                data: function (d) {
                    d.name = $('#name').val();
                    d._token = '{{csrf_token()}}'
    ...
    function delete_data(id) {
        swal({
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: 'get',
                    url: '{{url('admin/servingunit/delete')}}/' + id,
    ...
    $('#save_info').off().on('submit', function (event) {
        ...
        disableButton()
        $.ajax({
            url: "{{url('admin/servingunit/store')}}",
            type: "POST",
            ...
            error: function (response) {
                ...
                $('#error_add_name').text(response.responseJSON.errors.name);
                $('#error_add_status').text(response.responseJSON.errors.status);
            }
        });
    })
    ...
    function edit(id) {
        ...
        $.ajax({
            url: "{{url('admin/servingunit/edit')}}/" + id,
            type: "GET",
            success: function (response) {
                if (response) {
                    enableeButton()
    ...
    $('#update_form').off().on('submit', function (event) {
        ...
        disableButton()
        $.ajax({
            url: "{{url('admin/servingunit/update')}}",
            type: "POST",
            data: $(this).serializeArray(),
            success: function (response) {
                if (response.error) {
