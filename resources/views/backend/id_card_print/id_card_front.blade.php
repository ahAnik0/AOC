...
<div class="card main">
    <div class="container">
        ...
        <div class="image_container" style="display: flex;margin: 5px 8px 0 8px;height: 95px;padding:3px;">
            ...
            <img src="{{ asset('uploads/member_Photograph/' . $member->photo) }}" alt="profile photo"
                 style="float: right;width: 83px;height: 90px;border-radius: 3px"/>
        </div>
...
{{--                    {{ \Carbon\Carbon::parse($member->expire_date)->format('Y M d') }} <br/>--}}
            ...
            <div class="right_side_content" style="width: 47%;float: right;overflow-wrap: break-word;">
                <div class="name_container" style="font-weight: bolder;color: #000000;font-size: .6rem;">
                    @if (!$member->parent_member_id)
                        <p style="margin: 0">
                            {{ $member->designation->short_name !== 'NULL' ? $member->designation->short_name . ' ' : '' }}
                            {{ $member->fullname }}
                        </p>
                    @else
                        {{ $member->fullname }}
        ...
        <div class="sign_container" style="height: 44px;font-size: 11px;margin: 0 8px 0 8px">
            <div class="left_sigh" style="width: 40%;float: left;">
                <img src="{{ asset('uploads/member_sigh/' . $member->signature) }}" alt="profile photo"
                     style="height: 25px;width: 60%;margin-left: 20px;margin-bottom: 2px;"/>
