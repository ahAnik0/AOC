<script nonce="{{ csp_nonce() }}">
    ...
    $(function () {
        var table = $('.yajra-datatable').DataTable({
            ...
            ajax: {
                url: "{{ url('admin/library/library_booking_search') }}",
                type: 'POSt',
                data: function (d) {
    ...
    function return_book(id) {
        swal({
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: 'get',
                    url: '{{url('admin/library/change_booking_status')}}/' + id,
                    success: function (response) {
                        if (response) {
                            if (response.permission == false) {
                                toastr.warning('you dont have that Permission', 'Permission Denied');
                            } else {
                                toastr.success('Status Changed Successful', 'returned');
                                $('.yajra-datatable').DataTable().ajax.reload(null, false);
                            }
                        }
                    }
    ...
    $('#save_cancel_info').off().on('submit', function (event) {
        ...
        disableButton()
        $.ajax({
            url: "{{url('admin/library/cancel_booking')}}",
            type: "POST",
